...
```yaml
  etcd:
    ...
    container_name: eikohelp-milvus-etcd
    environment:
  ...
  minio:
    ...
    container_name: eikohelp-milvus-minio
    environment:
      MINIO_ACCESS_KEY: minioadmin
      MINIO_SECRET_KEY: minioadmin
    volumes:
      - eikohelp-milvus-minio:/minio_data
  ...
  milvus-standalone:
    ...
    container_name: eikohelp-milvus
    command: ["milvus", "run", "standalone"]
    environment:
      ETCD_ENDPOINTS: etcd:2379
      MINIO_ADDRESS: minio:9000
    volumes:
      - eikohelp-milvus-data:/var/lib/milvus
    healthcheck:
...
底部 volumes 追加:
```yaml
  eikohelp-milvus-etcd:
  eikohelp-milvus-minio:
  eikohelp-milvus-data:
```
...
```bash
docker exec -i eikohelp-mysql mysql -uroot -proot eikohelp -e "SELECT id,source,LEFT(raw_question,20),LEFT(reason,30) FROM low_confidence_questions ORDER BY id DESC LIMIT 5;"
```
