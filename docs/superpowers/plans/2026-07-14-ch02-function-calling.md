...
- 测试:可单测代码走 TDD(先红后绿);纯 Prompt/模型行为走标注样例评估。DB 测试连 Docker MySQL 的 `eikohelp_test` 库(不用 SQLite——ENUM/`ON UPDATE`/JSON 行为不一致)。
...
services:
  mysql:
    image: mysql:8.0
    container_name: eikohelp-mysql
    environment:
      MYSQL_ROOT_PASSWORD: root
      MYSQL_DATABASE: eikohelp
    ports:
      - "3306:3306"
    volumes:
      - eikohelp-mysql-data:/var/lib/mysql
...
docker compose exec -T mysql mysql -uroot -proot eikohelp -e "SHOW TABLES;"
```
...
```python
    database_url: str = "mysql+asyncmy://root:root@localhost:3306/eikohelp"
    test_database_url: str = "mysql+asyncmy://root:root@localhost:3306/eikohelp_test"
```
`.env.example` 追加:
```
DATABASE_URL=mysql+asyncmy://root:root@localhost:3306/eikohelp
TEST_DATABASE_URL=mysql+asyncmy://root:root@localhost:3306/eikohelp_test
```
...
async def _test_engine():
    # 用 server-url(不带库名)建 eikohelp_test 库
    server_url = settings.test_database_url.rsplit("/", 1)[0]
    admin = create_async_engine(server_url, isolation_level="AUTOCOMMIT")
    async with admin.connect() as conn:
        await conn.execute(text("DROP DATABASE IF EXISTS eikohelp_test"))
        await conn.execute(text("CREATE DATABASE eikohelp_test CHARACTER SET utf8mb4"))
    await admin.dispose()
...
```makefile
seed:
	docker compose exec -T mysql mysql -uroot -proot eikohelp < sql/ch02-seed.sql
```
...
```bash
make seed
docker compose exec -T mysql mysql -uroot -proot eikohelp -e "SELECT COUNT(*) FROM faq;"
```
...
Run:
```bash
docker compose exec -T mysql mysql -uroot -proot eikohelp -e "SELECT question FROM faq WHERE question LIKE '%退货%';"
docker compose exec -T mysql mysql -uroot -proot eikohelp -e "SELECT question FROM faq WHERE question LIKE '%鞋子%';"
```
