...
- 测试命令:`uv run pytest <path> -v`;全量 `make test`。测试连真 MySQL 测试库(docker eikohelp-mysql 需在跑)。
...
Run: `docker exec -i eikohelp-mysql mysql --default-character-set=utf8mb4 -uroot -proot eikohelp < sql/ch08-ddl.sql && docker exec -i eikohelp-mysql mysql -uroot -proot eikohelp -e "DESC tool_audit_logs;"`
Expected: 表结构 13 列。
...
make mcp-down
```
Expected: 清单含内置 4 + MCP 3;调用成功且 content 是翻译后的中文字段。(此冒烟会向 eikohelp 库落审计——dev 库已建表,正好人肉 SELECT 验一条。)
