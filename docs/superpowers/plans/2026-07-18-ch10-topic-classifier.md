...
- 测试命令:`PYTHONPATH=. uv run pytest tests/... -v`(需本地 mysql,test 库 `eikohelp_ch02_test` 由 conftest 自建);LLM 相关脚本需上游可用(`.env` 里填好 `CHAT_*` 三项)。
...
Run: `mysql -uroot -proot -h127.0.0.1 eikohelp < sql/ch10-ddl.sql && mysql -uroot -proot -h127.0.0.1 eikohelp -e "SHOW CREATE TABLE topic_classifications\G" | head -5`
Expected: 表结构输出
...
app = FastAPI(title="eikohelp ch10 topic classifier")
...
幂等:已归类的行(LEFT JOIN 命中)不重复归。定时跑 cron 示例:
  0 3 * * * cd /path/to/eikohelp && make classify-pool >> log/classify-pool.log 2>&1"""
import argparse
import asyncio
...
```bash
make classify-pool
mysql -uroot -proot -h127.0.0.1 eikohelp -e "SELECT COUNT(*) FROM topic_classifications;"
