...
```bash
docker exec -i eikohelp-mysql mysql --default-character-set=utf8mb4 -uroot -proot eikohelp < sql/ch09-ddl.sql
...
```yaml
      LANGFUSE_INIT_ORG_ID: eikohelp
      LANGFUSE_INIT_ORG_NAME: EikoHelp
      LANGFUSE_INIT_PROJECT_ID: eikohelp-ch09
      LANGFUSE_INIT_PROJECT_NAME: eikohelp
      LANGFUSE_INIT_PROJECT_PUBLIC_KEY: pk-lf-eikohelp-local
      LANGFUSE_INIT_PROJECT_SECRET_KEY: sk-lf-eikohelp-local
      LANGFUSE_INIT_USER_EMAIL: admin@eikohelp.local
      LANGFUSE_INIT_USER_NAME: admin
      LANGFUSE_INIT_USER_PASSWORD: eikohelp123
```
...
运行:make flywheel(需 mysql + 上游可用)。定时跑给 cron 示例:
  */30 * * * * cd /path/to/eikohelp && make flywheel >> log/flywheel.log 2>&1
"""
import asyncio
import sys
...
运行:make eval-flywheel(TRIGGER=手动|定时,默认手动)。cron 示例:
  0 6 * * * cd /path/to/eikohelp && make eval-flywheel TRIGGER=定时 >> log/eval.log 2>&1
