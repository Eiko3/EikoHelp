...
# -p 独立 project 名:不加会与主 compose 同落 eikohelp project,两边的 minio 服务同名相撞
langfuse-up:
	docker compose -p eikohelp-langfuse -f docker-compose.langfuse.yml up -d
	@echo "Langfuse 起中: http://localhost:3000 (admin@eikohelp.local / eikohelp123)"
	@echo "首次就绪约 2-3 分钟;key 已 headless 预置,写 .env:"
	@echo "  LANGFUSE_PUBLIC_KEY=pk-lf-eikohelp-local"
	@echo "  LANGFUSE_SECRET_KEY=sk-lf-eikohelp-local"
	@echo "  LANGFUSE_BASE_URL=http://localhost:3000"

langfuse-down:
	docker compose -p eikohelp-langfuse -f docker-compose.langfuse.yml down
...
seed:
	docker exec -i eikohelp-mysql mysql --default-character-set=utf8mb4 -uroot -proot eikohelp < sql/ch02-seed.sql
...
seed-conv:
	docker exec -i eikohelp-mysql mysql --default-character-set=utf8mb4 -uroot -proot eikohelp < sql/ch03-seed.sql
...
kb-reset:
	docker exec -i eikohelp-mysql mysql -uroot -proot eikohelp -e "SET FOREIGN_KEY_CHECKS=0; DELETE FROM knowledge_chunks; DELETE FROM qa_extraction_staging; SET FOREIGN_KEY_CHECKS=1;"
