...
**浏览器里的电商智能客服 Agent。** FastAPI + LangGraph + MySQL + Milvus：能查订单物流、答政策 FAQ、走退款子流程、从对话里挖知识补库，还带一个微调出来的主题分类器。《AI Agent 智能客服实战》的配套源码，代码随课程一章章长出来：ch01 一个纯对话接口起步，到 ch10 收尾是下面这套东西。
...
编辑 `.env`，填三组密钥（用同一家上游就填同一个值，默认按硅基流动给的，注册一个账号就能跑完整门课）：
...
- **spec 和 plan 留在仓库里。** `docs/superpowers/` 是各章的设计文档，课程实战篇教的就是这套流程，留着当范本
